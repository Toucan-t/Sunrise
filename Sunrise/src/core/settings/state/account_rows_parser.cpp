...
bool Parser::profile_items(state::AccountState& output) noexcept {
    ...
    for (;;) {
        ...
        bool hasInstanceSoid = false;
        bool hasMutationSerial = false;
        for (;;) {
            std::string_view key;
            ...
            if (key == "instance_soid") {
                if (hasInstanceSoid || !unsigned_value(value)) {
                    return false;
                }
                item.instanceSoid = value;
                hasInstanceSoid = true;
                ...
                if (hasMutationSerial || !unsigned_integer(value)
                    || value > (std::numeric_limits<std::int32_t>::max)()) {
                    return false;
                }
                item.mutationSerial = static_cast<std::int32_t>(value);
                hasMutationSerial = true;
