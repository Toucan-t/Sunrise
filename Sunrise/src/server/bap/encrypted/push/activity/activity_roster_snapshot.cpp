...
RosterOutcome build_roster_snapshot(Session& session,
    ...
    // First publication registers the group keys. The client answers with its normal activity-state
    // refresh, and the subsequent send carries the authority/object state. This behavior is part of
    // the current roster startup path; only the old diagnostics around it were removed.
