...
/** Resolves one profile mod/shader action source into the shared Family-4 instance schema. */
...
/** Appends every nonzero-SOID profile action source after character-owned item residents. */
[[nodiscard]] bool append_profile_items(
    Scratch& scratch,
    std::span<std::byte> rawStorage,
    std::uint32_t itemInstanceObjectId,
