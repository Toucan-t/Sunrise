...
bool append_banner_notification(Scratch& scratch,
    ...
    after = before;
    // With no real selection, prepare_banner() emits the reserved presentation-only Guardian. That
    // primes the same Family-0 renderer the normal first character pick would otherwise initialize.
    ...
    const std::uint64_t delivered =
        selected != 0 ? selected : snapshot::kCreatorBootstrapCharacterSoid;
    ...
    if (selected == 0) {
        core::log::write(core::log::Channel::server,
                         core::log::Level::info,
                         "ev=queuez stage=creator_bootstrap result=published");
    }
