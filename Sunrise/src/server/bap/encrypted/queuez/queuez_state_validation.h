...
/**
 * Stages an incremental that only adds new resident Family-4 objects.
 * Used by native character creation to install the new equipment-instance SOIDs without
 * republishing the large account body.
 */
[[nodiscard]] bool stage_family4_additions(const SessionState& before,
                                           const middleware::queuez::Family& family,
                                           SessionState& after) noexcept;

/**
 * Stages one mixed Family-4 incremental containing resident updates, new residents, and/or
 * empty-payload releases. Object order is preserved while the manifest is updated atomically.
 */
[[nodiscard]] bool stage_family4_mutation(const SessionState& before,
                                          const middleware::queuez::Family& family,
                                          SessionState& after) noexcept;
