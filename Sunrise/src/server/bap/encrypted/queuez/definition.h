...
struct StagedPublication {
    ...
    bool hasState{};
    /**
     * Opcode 501 created a character before this peer had a resident Queuez ladder, so staging
     * emitted a fresh Family-3 roster plus its Family-4 companion from the account root.
     */
    bool createdCharacterBootstrap{};
    /** Native equip/unequip published Family 4 and still owes Family 3/0 appearance mirrors. */
    bool armsEquipmentMirrors{};
    /** Selected character whose roster/banner mirrors must follow the equipment mutation. */
    std::uint64_t equipmentMirrorCharacterSoid{};
