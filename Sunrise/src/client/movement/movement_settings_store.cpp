/**
 * Settings for movement features added on top of the fork's existing Teleport module. Keeping this
 * separate avoids changing teleport.json or making the reverted upstream movement refactor a
 * dependency of the character/inventory work on this branch.
 */
...
[[nodiscard]] bool valid(const Settings& settings) noexcept {
    return settings.noclipToggleKey <= kMaximumVirtualKey
